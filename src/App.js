...
function App() {
  ...
  const swichNetwork = async (chainId) => {
    console.log(chainId,"network switch");
  ...
  const connector = async () => {
    try {
      ...
      await web3.eth.requestAccounts().then(async(result) => {
      await localStorage.setItem("userid", result[0]);
        setlogedin(true);
  ...
  const connectmass = async (net) => {
    if (window.web3.currentProvider) {
      const idn = await web3.eth.net.getId()
      if (idn === net) {
        await connector();
        
      } else {
        await swichNetwork(net);
      }
    } else {
  ...
  const sendTransaction = async () => {
    if (localStorage.getItem("userid") === null) {
      ...
      console.log(newAmountValue,"neeww");
      console.log(balance,"baalann");
      if (localStorage.getItem("userid") === null) {
      ...
      }else if(newAmountValue >= balance){
        toast.error(`You Dont have sufficient balance,Your current balance is ${balance}`)
      } 
      else {
        try {
          console.log(userAdd);
  ...
  return (
    <>
      ...
      <div className="btn-group ml-2">
        ...
        <div className="dropdown-menu">
          <li onClick={()=>{ connectmass(56)}} className="dropdown-item">BNB</li>
          <li onClick={()=>{ connectmass(3)}} className="dropdown-item">Ropsten </li>
          <li onClick={()=>{ connectmass(4)}} className="dropdown-item">Rinke </li>
        </div>
      </div>
      <div className="mb-3">
        <h4>Send Coins</h4>
